...
all_salaries.extend(extra_salaries)

# sort the salaries 
sorted_salaries = all_salaries.copy()
sorted_salaries.sort(reverse=True)

# Max & min salaries
maximum_salary = max(all_salaries)

minimum_salary = min(all_salaries)

# top & bottom 5 salaries

top_five = sorted_salaries[:5]

bottom_five = sorted_salaries[-5:]

# avrage salary

avrage_salary = sum(all_salaries) / len(all_salaries)

# number of people with salary = x

num_people = all_salaries.count(15500)

# printing the result

print(f"Maximum salary: {maximum_salary} EGP\n")
print(f"Top 5 salaries: {top_five} EGP\n")
print("-" * 20)


print(f"Minimum salary: {minimum_salary} EGP\n")
print(f"Bottom 5 salaries: {bottom_five} EGP\n")
print("-" * 20)

print(f"Average salary: {avrage_salary:.0f} EGP")

print("-" * 20)

print(f"Number of people with salary = 15,500 EGP: {num_people}")



# sorted_salary = salary.copy()
# sorted_salary.sort(reverse=True)
# print(sorted_salary)
# # max salary 
# max_s = max(salary)
# # min salary 
# min_s = min(salary)
# # average saalary 
# avr = sum(salary) / len(salary)
# # highest 5 salaries
# top_five = sorted_salary[:5]

# # lowest 5 salaries
# min_five = sorted_salary[-5:]

# print(f"max salary: {max_s}\nmin salary: {min_s}\navrage salary: {avr:.0f}\nhighest 5 salaries: {top_five}\nlowest 5 salaries: {min_five}") 





https://www.programiz.com/online-compiler/9gfNkmjzeuqZF